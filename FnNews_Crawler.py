...
import time
from News_keyword import keywords, exclude_keywords  # keyword.py에서 키워드 가져오기# JSON 저장 폴더 설정
NEWS_JSON_DIR = 'news_json'
result_filename = os.path.join(NEWS_JSON_DIR, 'Fn_News.json')# 날짜 포맷팅 (모든 요일을 한국어로 변환)
...
today = today_dt.strftime(f'%Y년 %m월 %d일 {kor_day}')urls = ['https://www.fnnews.com/newsflash']
